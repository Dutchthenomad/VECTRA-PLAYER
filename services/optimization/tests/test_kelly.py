...
class TestKellyCriterion:
    ...
    def test_kelly_criterion_with_edge(self):
        ...
        # 25% win rate, 5:1 payout (breakeven is 20%)
        fraction = kelly_criterion(win_rate=0.25, payout=5.0)
    ...
    def test_kelly_criterion_no_edge(self):
        ...
        # 15% win rate, 5:1 payout (below 20% breakeven)
    ...
    def test_fractional_kelly(self):
        ...
        full = kelly_criterion(win_rate=0.25, payout=5.0)
        quarter = fractional_kelly(win_rate=0.25, fraction=0.25, payout=5.0)
    ...
    def test_calculate_edge(self):
        ...
        edge = calculate_edge(win_rate=0.25, payout=5.0)

        assert isinstance(edge, dict)
        assert "expected_value" in edge
        assert "breakeven_win_rate" in edge
        assert edge["edge_exists"] is True
        assert edge["breakeven_win_rate"] == pytest.approx(1 / 5, rel=0.01)
