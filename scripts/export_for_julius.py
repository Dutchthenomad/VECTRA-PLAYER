...
for _, row in df_raw.iterrows():
    ...
    for bet in game.get('globalSidebets', []):
        sidebets.append({
            ...
            'payout_sol': bet.get('betAmount', 0) * bet.get('xPayout', 0) if peak >= bet.get('xPayout', 999) else 0,
            'profit_sol': (bet.get('betAmount', 0) * bet.get('xPayout', 0) - bet.get('betAmount', 0))
...
print(f"2. Upload BOTH files:")
print(f"   - {output_file_1.name}")
print(f"   - {output_file_2.name}")
print(f"\n3. Try these questions:")
print(f"   • 'What's the sidebet win rate by target multiplier?'")
print(f"   • 'Show correlation between game peak and number of sidebets'")
print(f"   • 'Plot profit/loss distribution across players'")
print(f"   • 'Which entry ticks have the highest win rate?'")
print(f"   • 'Show price trajectory patterns for games with high sidebet activity'")
print()
