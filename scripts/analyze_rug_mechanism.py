...
for _, row in df.iterrows():
    ...
    rug_drops.append({
        ...
        'drop_percent': (max_drop / prices[max_drop_idx - 1] * 100) if prices[max_drop_idx - 1] > 0 else 0,
...
print(f"{'Game':<10} {'Before':<10} {'After':<10} {'Drop %':<10} {'Rug Tick':<12} {'Ticks After':<12}")
print('-' * 70)

for rug in rug_drops[:10]:
    print(f"{rug['game_id']:<10} {rug['price_before_rug']:<10.4f} {rug['price_after_rug']:<10.6f} "
...
print(f"  Rugs below 0.02x: {len(rugs_below_02)} ({len(rugs_below_02)/len(rug_drops)*100:.1f}%)")

# House take calculation
print(f'\n💰 House Liquidation Calculation:')
print(f"  When rug happens at 1.0x → 0.02x:")
print(f"    Price drops: 98% instantly")
print(f"    All positions @ 1.0x are liquidated")
print(f"    House keeps: 100% of liquidated value")
print(f"  ")
...
print(f"  ")
print(f"  Total house edge: Liquidation value + tick fees")
