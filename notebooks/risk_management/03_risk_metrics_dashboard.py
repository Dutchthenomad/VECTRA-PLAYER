...
def print_risk_dashboard(dashboard: RiskMetricsDashboard):
    ...
    print("\n📈 RISK-ADJUSTED RETURNS")
    print(f"  Sharpe Ratio:      {dashboard.sharpe_ratio:>8.2f}  {'✅' if dashboard.sharpe_ratio > 1.0 else '⚠️'}")
    print(f"  Sortino Ratio:     {dashboard.sortino_ratio:>8.2f}  {'✅' if dashboard.sortino_ratio > 1.5 else '⚠️'}")
    print(f"  Calmar Ratio:      {dashboard.calmar_ratio:>8.2f}  {'✅' if dashboard.calmar_ratio > 1.0 else '⚠️'}")
    ...
    print("\n💰 TRADE QUALITY")
    print(f"  Profit Factor:     {dashboard.profit_factor:>8.2f}  {'✅' if dashboard.profit_factor > 1.5 else '⚠️'}")
...
def plot_risk_dashboard(dashboard: RiskMetricsDashboard, equity_curve: List[float]):
    ...
    ax5.text(0.5, 0.5, f"Longest Win Streak:\n{dashboard.longest_win_streak} trades\n\n"
    ...
    ax6.text(0.5, 0.5, f"Longest Loss Streak:\n{dashboard.longest_loss_streak} trades\n\n"
                       f"Avg Loss Streak:\n{dashboard.avg_loss_streak:.1f} trades",
             fontsize=12, ha='center', va='center', transform=ax6.transAxes,
    ...
    """
    ax7.text(0.1, 0.5, grade_text, fontsize=11, family='monospace',
