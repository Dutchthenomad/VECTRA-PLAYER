...
from bayesian_sidebet_analysis import BayesianSurvivalModel, extract_features, load_game_data
...
class RiskManager:
    ...
    def calculate_position_size(self, p_win: float) -> float:
        ...
        # Apply limits
        max_bet = min(
            self.config.max_bet_size, self.state.bankroll * (self.config.max_bet_pct / 100)
        )
    ...
    def _update_trading_state(self):
        ...
        if self.state.current_drawdown_pct >= self.config.max_drawdown_pct:
            ...
            print(
                f"⚠️  PAUSED: Drawdown {self.state.current_drawdown_pct:.1f}% >= {self.config.max_drawdown_pct}%"
            )
            return
        ...
        if self.state.trading_state == TradingState.PAUSED:
            ...
            recovery_trades = sum(
                1 for t in self.state.trade_history[-self.config.recovery_trades :] if t["outcome"]
            )
    ...
    def get_summary(self) -> pd.DataFrame:
        ...
        summary = {
            ...
            "Win Rate (%)": (self.state.total_wins / self.state.total_trades * 100)
            if self.state.total_trades > 0
            else 0,
            "Current Bankroll": self.state.bankroll,
            "Peak Bankroll": self.state.peak_bankroll,
            "Total Return (%)": (
                (self.state.bankroll - self.state.equity_curve[0])
                / self.state.equity_curve[0]
                * 100
            ),
...
def backtest_risk_system(
    ...
    for _, game in test_games.iterrows():
        ...
        # Should we bet?
        should_bet, _reason = risk_mgr.should_place_bet(p_win)
...
def plot_backtest_results(risk_mgr: RiskManager):
    ...
    for i, trade in enumerate(trades_df.itertuples()):
        color = {"active": "green", "reduced": "yellow", "recovery": "orange", "paused": "red"}.get(
            trade.state, "gray"
        )
    ...
    ax.axhline(
        -risk_mgr.config.reduce_size_dd_pct, color="orange", linestyle="--", label="Reduce Size"
    )
    ax.axhline(
        -risk_mgr.config.max_drawdown_pct, color="red", linestyle="--", label="Pause Trading"
    )
    ax.set_xlabel("Trade #")
    ...
    colors_list = [
        "green" if s == 0 else "yellow" if s == 1 else "orange" if s == 2 else "red"
...
def run_comprehensive_analysis(save_plots: bool = True):
    ...
    risk_mgr, trades_df = backtest_risk_system(
        games_df, survival_model, config, initial_bankroll=1.0, num_games=200
    )
