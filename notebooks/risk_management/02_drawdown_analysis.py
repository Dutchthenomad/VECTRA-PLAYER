...
def plot_monte_carlo_results(mc_results: Dict, initial_bankroll: float = 1.0):
    ...
    ax.axvline(mc_results['mean_final'], color='red', linestyle='--', label=f'Mean: {mc_results["mean_final"]:.2f}')
    ax.axvline(mc_results['median_final'], color='green', linestyle='--', label=f'Median: {mc_results["median_final"]:.2f}')
    ax.axvline(mc_results['p5_final'], color='orange', linestyle=':', label=f'5th %ile: {mc_results["p5_final"]:.2f}')
    ...
    ax.axvline(mc_results['mean_mdd'], color='blue', linestyle='--', label=f'Mean: {mc_results["mean_mdd"]:.1f}%')
    ax.axvline(mc_results['p95_mdd'], color='orange', linestyle='--', label=f'95th %ile: {mc_results["p95_mdd"]:.1f}%')
    ax.axvline(mc_results['p99_mdd'], color='red', linestyle='--', label=f'99th %ile: {mc_results["p99_mdd"]:.1f}%')
    ...
    if mc_results['recovery_times']:
        ...
        ax.axvline(mc_results['mean_recovery'], color='red', linestyle='--', 
        ...
        ax.text(0.5, 0.5, 'No recoveries observed', 
...
def run_drawdown_analysis(save_plots: bool = True):
    ...
    for name, bet_frac in strategies.items():
        ...
        print(f"    Final bankroll: {mc_result['mean_final']:.3f} ± {mc_result['median_final']:.3f}")
