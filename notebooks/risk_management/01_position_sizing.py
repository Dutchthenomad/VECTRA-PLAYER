...
from bayesian_sidebet_analysis import BayesianSurvivalModel, load_game_data
...
def volatility_adjusted_kelly(
    p_win: float, payout_multiplier: int, volatility: float, base_vol: float = 0.3
) -> float:
    """
...
def compare_strategies(
    ...
    results.append(
        simulate_betting_strategy(
            lambda bankroll, p_win: volatility_adjusted_kelly(
                p_win, payout_multiplier, win_prob_vol
            )
            * bankroll,
            win_probabilities,
            actual_outcomes,
...
def plot_strategy_comparison(results: list[PositionSizingResult], initial_bankroll: float = 1.0):
    ...
    width = 0.35
    ax.bar(
        x - width / 2, [r.max_drawdown for r in results], width, label="Max Drawdown (%)", alpha=0.7
    )
...
def run_position_sizing_analysis(save_plots: bool = True):
    ...
    comparison_df, results = compare_strategies(
        win_probabilities, actual_outcomes, payout_multiplier=5, initial_bankroll=1.0
    )
