...
def extract_features(prices: List[float], tick: int) -> GameFeatures:
    ...
    is_rising = len(recent_prices) > 1 and all(recent_prices[i] <= recent_prices[i + 1] for i in range(len(recent_prices) - 1))
    is_falling = len(recent_prices) > 1 and all(recent_prices[i] >= recent_prices[i + 1] for i in range(len(recent_prices) - 1))
    
    # Rapid changes
    if tick >= 3:
        change_3_tick = (prices[tick] - prices[tick - 3]) / prices[tick - 3] if prices[tick - 3] > 0 else 0
...
def create_training_dataset(games_df: pd.DataFrame, 
                           sidebet_window: int = 40) -> pd.DataFrame:
    """
...
class BayesianSurvivalModel:
    ...
    def _compute_baseline_hazard(self) -> np.ndarray:
        ...
        hazard = np.divide(rug_counts, survival_counts, 
                          out=np.zeros_like(rug_counts), 
                          where=survival_counts > 0)
    ...
    def predict_rug_probability(self, 
                                current_tick: int, 
                                window: int = 40,
                                features: Optional[GameFeatures] = None) -> float:
        """
        ...
        S_future = self.survival_function(len(self.hazard_rates))[min(current_tick + window, len(self.hazard_rates) - 1)]
...
def expected_value(p_win: float, 
                  payout_multiplier: int = 5,
                  bet_amount: float = 0.001) -> float:
    """
...
def kelly_criterion(p_win: float, 
                   payout_multiplier: int = 5) -> float:
    """
