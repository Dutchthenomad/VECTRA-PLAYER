...
class MonteCarloSimulator:
    ...
    def run(
        self, num_iterations: int = 10000, num_games: int = 500, win_rate: float = 0.185
    ) -> dict:
        """
    ...
    def _calculate_bet_size(
        self, bankroll: float, win_streak: int, games_played: int, volatility: float
    ) -> float:
        ...
        if mode == ScalingMode.FIXED:
            ...
            multiplier = min(
                self.config.win_streak_multiplier**win_streak, self.config.max_streak_multiplier
            )
    ...
    def _aggregate_results(
        self, final_bankrolls: list, max_drawdowns: list, games_to_ruin: list, num_iterations: int
    ) -> dict:
    ...
    def _calc_calmar(
        self, final_bankrolls: np.ndarray, max_drawdowns: np.ndarray, initial: float
    ) -> float:
...
if __name__ == "__main__":
    ...
    strategies = [
        ...
        ("Kelly 0.25", ScalingMode.KELLY),
        ("Theta-Bayesian", ScalingMode.THETA_BAYESIAN),
        ("Anti-Martingale", ScalingMode.ANTI_MARTINGALE),
    ]
    ...
    for name, mode in strategies:
        ...
        sim = MonteCarloSimulator(config, seed=42)
        results = sim.run(num_iterations=1000, num_games=500, win_rate=0.185)
        ...
        print(f"  Max DD: {results['drawdown']['mean_max_drawdown']:.1%}")
