...
def find_optimal_entry_window(
    ...
    return {
        ...
        "positive_edge_range": (int(positive_edge_ticks[0]), int(positive_edge_ticks[-1])),
        "ev_per_bet": float(max_edge * 4),  # Expected value per unit bet
    }


def should_place_sidebet(
    current_tick: int, durations: np.ndarray, window_size: int = 40, min_edge: float = 0.02
) -> dict:
    """
...
if __name__ == "__main__":
    ...
    # Generate sample data (simulated game durations)
    np.random.seed(42)
    # Log-normal distribution roughly matches real game durations
    durations = np.random.lognormal(mean=5.0, sigma=0.6, size=500).astype(int)
    durations = np.clip(durations, 50, 1000)  # Clip to reasonable range

    print(f"\nDataset: {len(durations)} games")
    print(f"Duration range: {durations.min()} - {durations.max()} ticks")
    print(f"Mean duration: {durations.mean():.0f} ticks")
    print(f"Median duration: {np.median(durations):.0f} ticks")

    # Survival curve
    print("\nSurvival Analysis:")
    survival = compute_survival_curve(durations)
    for tick in [100, 150, 200, 250, 300]:
        idx = np.searchsorted(survival["times"], tick)
        if idx < len(survival["survival"]):
            print(f"  S({tick}): {survival['survival'][idx]:.1%} survive past tick {tick}")

    # Conditional probabilities
    print("\nConditional Probabilities (40-tick window):")
    cond_probs = compute_conditional_probability(durations)
    for tick in [100, 150, 200, 250, 300]:
        print(f"  P(rug in 40 | t>{tick}): {cond_probs[tick]:.1%}")

    # Optimal entry
    print("\nOptimal Entry Analysis:")
    optimal = find_optimal_entry_window(durations)
    if optimal.get("optimal_entry_tick"):
        print(f"  Optimal entry tick: {optimal['optimal_entry_tick']}")
        print(f"  Win rate at optimal: {optimal['win_rate_at_optimal']:.1%}")
        print(f"  Edge at optimal: {optimal['edge_at_optimal']:.1%}")
        print(f"  Positive edge range: {optimal['positive_edge_range']}")
        print(f"  EV per bet: {optimal['ev_per_bet']:.3f}")
