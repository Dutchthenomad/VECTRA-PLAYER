...
def fractional_kelly(win_rate: float, fraction: float = 0.25, payout: float = 5.0) -> float:
    """
...
def calculate_all_variants(
    win_rate: float, bankroll: float, payout: float = 5.0
...
def recommend_bet_size(
    win_rate: float, bankroll: float, payout: float = 5.0, risk_tolerance: str = "moderate"
) -> dict:
    """
...
def suggest_kelly_sizing(
    win_rate: float, initial_balance: float, num_bets: int, kelly_fraction: float = 0.25
) -> list[float]:
    """
...
if __name__ == "__main__":
    ...
    # Test with 20% win rate (above breakeven)
    win_rate = 0.20
    bankroll = 0.1  # 0.1 SOL

    print(f"\nWin Rate: {win_rate:.1%}, Bankroll: {bankroll} SOL")

    # Full Kelly
    full = kelly_criterion(win_rate)
    print(f"Full Kelly: {full:.2%} of bankroll = {bankroll * full:.4f} SOL")

    # Quarter Kelly (recommended)
    quarter = fractional_kelly(win_rate, 0.25)
    print(f"Quarter Kelly: {quarter:.2%} of bankroll = {bankroll * quarter:.4f} SOL")

    # All variants
    print("\nAll Kelly Variants:")
    variants = calculate_all_variants(win_rate, bankroll)
    for name, result in variants.items():
        print(f"  {name}: {result.fraction:.2%} = {result.bet_size:.4f} SOL ({result.risk_level})")

    # Edge calculation
    print("\nEdge Analysis:")
    edge = calculate_edge(win_rate)
    print(f"  Breakeven: {edge['breakeven_win_rate']:.2%}")
    print(f"  Actual: {edge['actual_win_rate']:.2%}")
    print(f"  Edge: {edge['edge_pct']:.1f}%")
    print(f"  EV per bet: {edge['expected_value']:.3f}")
