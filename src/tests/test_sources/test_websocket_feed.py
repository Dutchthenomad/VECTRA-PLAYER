...
from sources import GameSignal, GameStateMachine, WebSocketFeed
...
class TestGameStateMachine:
    ...
    def test_rug_event_2_detection(self):
        ...
        machine.process({"active": True, "rugged": True, "gameId": "game-123", "gameHistory": [{}]})
    ...
    def test_legal_state_transitions(self):
        ...
        result1 = machine.process(
            {"active": True, "tickCount": 0, "rugged": False, "gameId": "game-1"}
        )
        assert result1["phase"] == "GAME_ACTIVATION"

        result2 = machine.process(
            {"active": True, "tickCount": 1, "rugged": False, "gameId": "game-1"}
        )
    ...
    def test_tick_regression_detection(self):
        ...
        # First tick
        machine.process({"active": True, "tickCount": 50, "rugged": False, "gameId": "game-1"})
...
class TestWebSocketFeed:
    ...
    def test_player_identity_event_emission(self, mock_socketio):
        ...
        feed._emit_event(
            "player_identity", {"player_id": "did:privy:test123", "username": "TestPlayer"}
        )
