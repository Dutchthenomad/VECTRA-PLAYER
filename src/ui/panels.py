...
class StatusPanel(Panel):
    ...
    def _create_widgets(self):
        ...
        self.balance_label = ttk.Label(
            self.frame, text="Balance: 0.1000 SOL", font=("Arial", 12, "bold")
        )
        self.balance_label.pack(side=tk.LEFT, padx=10)

        # P&L display - semantic color applied in update()
        self.pnl_label = ttk.Label(
            self.frame, text="P&L: +0.0000 SOL", font=("Arial", 12), foreground="#00ff88"
        )
...
class ChartPanel(Panel):
    ...
    def _create_widgets(self):
        ...
        # Zoom buttons
        ttk.Button(controls_frame, text="🔍+ Zoom In", command=self.chart.zoom_in).pack(
            side=tk.LEFT, padx=5
        )

        ttk.Button(controls_frame, text="🔍- Zoom Out", command=self.chart.zoom_out).pack(
            side=tk.LEFT, padx=5
        )

        ttk.Button(controls_frame, text="↺ Reset Zoom", command=self.chart.reset_zoom).pack(
            side=tk.LEFT, padx=5
        )
...
class TradingPanel(Panel):
    ...
    def _create_widgets(self):
        ...
        # Title
        title = ttk.Label(
            self.frame, text="TRADING CONTROLS", font=("Arial", 11, "bold"), foreground="#00ff88"
        )
        title.pack(pady=(0, 5))
        ...
        ttk.Label(input_frame, text="Amount (SOL):", font=("Arial", 10)).pack(side=tk.LEFT, padx=5)

        self.amount_entry = ttk.Entry(input_frame, font=("Arial", 10), width=10)
        ...
        for amount in ["0.001", "0.005", "0.010", "0.050"]:
            ttk.Button(
                quick_frame, text=amount, command=lambda a=amount: self.set_amount(a), width=6
...
class BotPanel(Panel):
    ...
    def _create_widgets(self):
        ...
        # Title
        title = ttk.Label(
            self.frame, text="BOT CONTROLS", font=("Arial", 11, "bold"), foreground="#3366ff"
        )
        title.pack(pady=(0, 5))
        ...
        ttk.Label(strategy_frame, text="Strategy:", font=("Arial", 10)).pack(side=tk.LEFT, padx=5)
        ...
        self.strategy_dropdown.bind(
            "<<ComboboxSelected>>", lambda e: self.on_strategy_change(self.strategy_var.get())
        )

        # Bot status
        self.status_label = ttk.Label(
            self.frame, text="Status: Disabled", font=("Arial", 9), foreground="#666666"
        )
    ...
    def _update_ui(self):
        ...
        if self.bot_enabled:
            self.toggle_button.config(text="🤖 DISABLE BOT (SPACE)", bg="#00aa44")
            self.status_label.config(
                text=f"Status: Active ({self.strategy_var.get()})", foreground="#00ff88"
            )
        else:
            self.toggle_button.config(text="🤖 ENABLE BOT (SPACE)", bg="#444444")
            self.status_label.config(text="Status: Disabled", foreground="#666666")
...
class ControlsPanel(Panel):
    ...
    def _create_widgets(self):
        ...
        # Title
        title = ttk.Label(
            self.frame, text="REPLAY CONTROLS", font=("Arial", 11, "bold"), foreground="#ffcc00"
        )
        title.pack(pady=(0, 5))
        ...
        self.speed_label = ttk.Label(speed_frame, text="1.0x", font=("Arial", 10), width=5)
    ...
    def _update_ui(self):
        ...
        if self.is_playing:
            self.play_pause_button.config(text="⏸ PAUSE (P)", bg="#cc2244")
        else:
            self.play_pause_button.config(text="▶ PLAY (P)", bg="#00aa44")
