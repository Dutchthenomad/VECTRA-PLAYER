...
class BotConfigPanel:
    ...
    def _load_config(self) -> dict[str, Any]:
        ...
        if default_config.get("execution_mode") not in allowed_modes:
            logger.warning(
                f"Invalid execution_mode '{default_config.get('execution_mode')}', resetting to ui_layer"
            )
        ...
        if default_config.get("strategy") not in allowed_strategies:
            logger.warning(
                f"Invalid strategy '{default_config.get('strategy')}', resetting to conservative"
            )
    ...
    def _create_ui(self):
        ...
        self.default_balance_var = tk.StringVar(
            value=str(self.config.get("default_balance_sol", 0.01))
        )

        default_balance_entry = ttk.Entry(
            balance_frame, textvariable=self.default_balance_var, width=12
        )
        ...
        enable_checkbox = ttk.Checkbutton(
            enable_frame, text="Enable bot on startup", variable=self.bot_enabled_var
        )
        ...
        timing_frame = ttk.LabelFrame(
            main_frame, text="UI Button Timing (UI Layer Mode)", padding="10"
        )
        ...
        self.button_depress_duration_var = tk.IntVar(
            value=self.config.get("button_depress_duration_ms", 50)
        )

        depress_spinbox = ttk.Spinbox(
            timing_frame, from_=10, to=500, textvariable=self.button_depress_duration_var, width=10
        )
        ...
        pause_spinbox = ttk.Spinbox(
            timing_frame, from_=0, to=5000, textvariable=self.inter_click_pause_var, width=10
        )
