...
class ModernReplayerUI(ttk.Window):
    ...
    def create_status_bar(self):
        ...
        for label, value, color in metrics:
            ...
            ttk.Label(
                container, text=label, font=("Roboto", 8, "bold"), bootstyle="secondary"
            ).pack(anchor=W)
            ttk.Label(container, text=value, font=("Roboto", 12, "bold"), bootstyle=color).pack(
                anchor=W
            )
    ...
    def create_control_panel(self, parent):
        ...
        ttk.Button(pb_btns, text="⏮", bootstyle="secondary-outline", width=4).pack(
            side=LEFT, padx=2
        )
        ...
        for val in ["MIN", "1/2", "2X", "MAX"]:
            ttk.Button(quick_row, text=val, bootstyle="secondary-outline", width=5).pack(
                side=LEFT, padx=2, expand=YES
            )
        ...
        ttk.Combobox(
            bot_frame, values=["Sniper v1", "Trend Follower", "Scalper"], state="readonly"
        ).pack(fill=X, pady=5)
