...
class MenuBarBuilder:
    ...
    def __init__(
        self, root: tk.Tk, callbacks: dict[str, Callable], variables: dict[str, tk.BooleanVar]
    ):
        """
    ...
    def _build_bot_menu(self, menubar: tk.Menu):
        ...
        bot_menu.add_command(
            label="Configuration...", command=self._get_callback("show_bot_config")
        )
        bot_menu.add_command(
            label="Timing Metrics...", command=self._get_callback("show_timing_metrics")
        )
        bot_menu.add_separator()
        bot_menu.add_checkbutton(
    ...
    def _build_browser_menu(self, menubar: tk.Menu) -> tuple:
        ...
        browser_menu.add_command(
            label="Connect to Browser", command=self._get_callback("connect_browser")
        )
        # Note: Phase 9.3 BrowserBridge (CDP bridge) is canonical for production.
        # Phase 8.5 BrowserExecutor + connection dialog is legacy/deprecated.
        browser_menu.add_command(label="⚠ Legacy: BrowserExecutor dialog (deprecated)", state=tk.DISABLED)
        ...
        browser_menu.add_command(label="Profile: rugs_bot", state=tk.DISABLED)
        browser_menu.add_separator()
    ...
    def _build_view_menu(self, menubar: tk.Menu):
        ...
        for theme_id, theme_label in dark_themes:
            dark_theme_menu.add_command(
                label=theme_label, command=lambda t=theme_id: change_theme(t)
            )
        ...
        ui_style_menu.add_command(
            label="Modern (Game-Like)", command=lambda: self._get_callback("set_ui_style")("modern")
        )
