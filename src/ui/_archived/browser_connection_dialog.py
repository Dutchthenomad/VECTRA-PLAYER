...
class BrowserConnectionDialog:
    ...
    async def _connect_async(self, profile: str, navigate: bool):
        """
    ...
    def _start_connection(self):
        """Start connection using dedicated AsyncLoopManager."""
        ...
        if async_manager is None:
            logger.error("BrowserConnectionDialog: AsyncLoopManager not provided")
            self._connection_finished(False, "AsyncLoopManager not available")
            return

        # Capture Tk state on the Tk thread before running async work elsewhere
        profile = self.profile_var.get()
        navigate = bool(self.navigate_var.get())

        future = async_manager.run_coroutine(self._connect_async(profile=profile, navigate=navigate))
