    ...
    def decide(
        ...
        if sidebet is None and info["can_sidebet"]:
            if balance >= self.SIDEBET_AMOUNT:
                return decide_action("SIDE", self.SIDEBET_AMOUNT, f"Testing sidebet at tick {tick}")

        # PRIORITY 2: Trade normally
        if position is None and info["can_buy"]:
            if price < self.BUY_THRESHOLD and balance >= self.BUY_AMOUNT:
                return decide_action("BUY", self.BUY_AMOUNT, f"Entry at {price:.2f}x")
        ...
        if position is not None and info["can_sell"]:
            ...
            if pnl_pct > self.TAKE_PROFIT:
                return decide_action("SELL", None, "Quick profit")

        # Default: wait
        return decide_action("WAIT", None, "Waiting for sidebet opportunity")
