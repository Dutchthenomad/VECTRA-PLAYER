    ...
    def decide(
        ...
        if position is None and info["can_buy"]:
            if price < self.BUY_THRESHOLD and balance >= self.BUY_AMOUNT:
                return decide_action(
                    "BUY", self.BUY_AMOUNT, f"Entry at {price:.2f}x (low price, good entry point)"
                )
        ...
        if position is not None and info["can_sell"]:
            ...
            if pnl_pct > self.TAKE_PROFIT:
                return decide_action(
                    "SELL", None, f"Take profit at +{pnl_pct:.1f}% (target: {self.TAKE_PROFIT}%)"
                )
            ...
            if price > self.BUBBLE_EXIT:
                return decide_action(
                    "SELL", None, f"Exit at {price:.2f}x (bubble risk, take gains)"
                )
        ...
        if sidebet is None and info["can_sidebet"]:
            if tick > self.SIDEBET_TICK and balance >= self.SIDEBET_AMOUNT:
                return decide_action(
                    "SIDE", self.SIDEBET_AMOUNT, f"Sidebet at tick {tick} (late game rug risk)"
                )
        ...
        if position:
            pnl_pct = Decimal(str(position["current_pnl_percent"]))
            return decide_action(
                "WAIT", None, f"Holding position (Price: {price:.2f}x, P&L: {pnl_pct:.1f}%)"
            )
        else:
            return decide_action("WAIT", None, f"Waiting for entry (Price: {price:.2f}x too high)")
